...
Agent(PT_ANT), ant_timer_(this), dmux_(0), hello_timer_(this), neighbor_timer_(this),
proactive_timer_(this), router_cache_(this)
{
...
bind("num_nodes_y_", &num_nodes_y_); // number of nodes in column (for regular mesh
topology)
...
fclose(fp);
this->ant_nest_->pheromoneTable()->printPheromoneTable(); // call method to
...
fprintf(stdout, "AntHocNet::recv (type %d): direct %d, source %d == this %d ==
...
{
/** Add the IP Header.* TCP adds the IP header too, so to avoid setting it

twice, we check if* this packet is not a TCP or ACK segment.*/
...
ant_nest_->pheromoneTable()->lookup(ah->getDestinationAddres());
...
this->ant_nest_->pheromoneTable()->nextNeighborNode(ah->getDestinationAddres(), true);
...
stdout,
"forwarding backward antnet packet from %d source %d dest %d next
...
ch->addr_type() = NS_AF_INET; // used for routing protocols implemented as Internet
protocol
ih->ttl_ = NETWORK_DIAMETER;
#endif
...
AntBackPacket* ahbk = (AntBackPacket*) AntBackPacket::access(pq); // ant
...
double time = CURRENT_TIME - ah->getTimeStartAnt(); // trip time to
...
ah->getSorceAddress(), ah->getSeqNum(), last, ahbk->getDestinationAddres(), ahbk->sizeNodes());

} else if (addr() != last) {
fprintf(stdout, "aiii - %d (%d : %d) == %d (%d) - %d\n", addr(),

ah->getSorceAddress(), ah->getSeqNum(), last, ahbk->getDestinationAddres(), size);
...
if (ch->ptype() != PT_ANT) {
nsaddr_t dest = (ch->ptype() == PT_ANT) ? HDR_AHN(p)->getDestinationAddres() :
...
bool antType = ah->getAntType() == ANTTYPEREACTIVE || ah->getAntType() ==
...
// routing table is updated for all the destination nodes that are visited after the
neighbor node
// update pheromone value corresponding to neighbor node and destination nodes visited
thereafter
// update pheromone valu fro neighbor node and this destination node
//fprintf(stdout, "updateRegularPheromone -> source %d >> dest %d -- neigh (%d) ==
...
//fprintf(stdout, "mind >> AntHocNet::initializeRouterTable ->>>
nd->get_node_by_address(addr()) \n");
int num_nb = this->ant_nest_->sizeNeighbors();
//fprintf(stdout, "mind >> AntHocNet::initializeRouterTable ->>>
phTable->getNumNeighbors() \n");
for (int i = 0; i < NUM_NODES; i++)
{
if (addr() != i)
{
//fprintf(stdout, "mind >> AntHocNet::initializeRouterTable ->>> for %d:
...
double phvalue = 1;
// add routing table entry
this->ant_nest_->updateRegularPheromone(ah->getSorceAddress(), ah->getSorceAddress(),
phvalue);
...
/*fprintf(stdout, "%s (ant type %d) - seqnum: %d:%d >> addr (%d) >> recv (%d) >> neig (%d)
>> dest (%d) >< visited size %d -- *p %d \n", str, HDR_AHN(p)->getAntType(),
HDR_AHN(p)->getSorceAddress(), HDR_AHN(p)->getSeqNum(),

addr(), HDR_CMN(p)->prev_hop_, HDR_CMN(p)->next_hop(),
HDR_AHN(p)->getDestinationAddres(), HDR_AHN(p)->sizeNodes(), p);*/
}
...
for (MapAntPQueue_it it = ant_nest_->mapQueue.begin(); it != ant_nest_->mapQueue.end();
it++) {

nsaddr_t destination = (*it).first;
AntDestinations dests_regular =

this->ant_nest_->pheromoneTable()->getRegularDestinations();
...
agent->sendPRFA();
double interval = MinProactiveInterval + ((MaxProactiveInterval - MinProactiveInterval) *
Random::uniform());
assert(interval >= 0);
Scheduler::instance().schedule(this, &intr, interval);
}
...
agent->purgeQueue();
double interval = 0.5;//MinProactiveInterval + ((MaxProactiveInterval - MinProactiveInterval)
*Random::uniform());
assert(interval >= 0);
Scheduler::instance().schedule(this, &intr, interval);
